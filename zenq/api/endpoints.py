...
class points():
    ...
    def insert_facts(self):
        ...
        # Create dropdown widgets for the user to choose columns
        column_dropdowns = [widgets.Dropdown(options=available_columns, description=f'Map to {col}')
                            for col in ['customer_id', 'location_id', 'location_name', 'invoice_id', 'date', 'quantity', 'total_price']]
        ...
        def on_submit_button_clicked(b):
            with output:
                ...
                # Insert the data into the Facts table
                unique_facts = self.df.drop_duplicates(subset=['invoice_id'])
                for index, row in unique_facts.iterrows():
                    customer_id, location_id, location_name, invoice_id, date, quantity, total_price = row[column_names[0]], row[column_names[1]], row[column_names[2]], row[column_names[3]], row[column_names[4]], row[column_names[5]], row[column_names[6]]
                    try:
                        ...
                        self.session.commit()
                        print(f'Successfully inserted customer: {invoice_id}, {date}, {quantity}, {total_price}')
                    except IntegrityError:
                        self.session.rollback()
